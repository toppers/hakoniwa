...
箱庭WGにおける最近の活動の紹介や、最新の技術成果に関する解説を行います。TOPPERSプロジェクトに関する様々な活動をキャッチアップできる機会です。ぜひご参加ください。
  ...
  森　崇（(株)永和マネジメント）  
  概要：メタバースという言葉が流行っていますが、VR空間に入って新しい価値観や体験ができるようになりました。箱庭コンセプトを実現する技術として、このようなゲーム技術(VR/Photon)は重要な要素技術であり、今年度、箱庭ROSシミュレータに組み込むことに成功しました。箱庭ワールドに入ることで、複数の拠点にいる開発者が、箱庭VR空間でロボット開発できます。 本発表では、この開発環境の説明と共に、どうやって利用できるようになるか説明したいと思います。
  - 「mROS 2：ロボットソフトウェアの組込みデバイス向け軽量実行環境」  
  高瀬英希（東京大学）  
  概要：ロボットソフトウェアの開発を加速するプラットフォームであるROS（Robot Operating System）が注目を集めています。本発表では、ROS 2で採用されているDDS/RTPS通信を組込みデバイス上から実現できる実行環境であるmROS 2を紹介します。mROS 2のSTM32 NUCLEO-F767ZIボード向け実装では、TOPPERS/ASP3カーネルおよび組込み向けの軽量プロトコルスタックを採用しています。本技術は、通信性能に優れた分散ロボットシステムを実現できるのみならず、ROS 2通信技術のIoT分野への展開に向けて貢献できることを示します。
